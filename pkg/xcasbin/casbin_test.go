...
func init() {
    var c = conf.NewConfig("../../configs")
    db := orm.NewDB(c)
    enforcer = NewCasbin(db, c)
}
...
func Test2(t *testing.T) {
    ...
    // 批量添加 (要是有期中一条存在，就整批不会添加成功)
    ok, err := enforcer.AddPermissionsForUser("xxx", []string{"/api/v1/hello5", "GET", "PID1000"}, []string{"/api/v1/hello6", "GET", "PID1001"})
    fmt.Println(ok, err)
}
...
func Test5(t *testing.T) {
    _, err := enforcer.UpdatePolicy([]string{"admin", "/api/v1/test/query", "VIEW"}, []string{"admin", "/api/v1/test/query", "look"})
    if err != nil {
        panic(err)
    }
}
...
func Test6(t *testing.T) {
    ...
    _, err := enforcer.UpdateFilteredPolicies([][]string{{"admin", "/api/v3/hello", "POST"}}, 0, "bob", "data2")
    if err != nil {
        panic(err)
    }
}
...
func Test7(t *testing.T) {
    ...
    // 批量添加
    ret, _ := enforcer.AddGroupingPolicies([][]string{[]string{"tom", common.SuperAdmin}})
...
func Test9(t *testing.T) {
    ret, _ := enforcer.GetRolesForUser("tom")
    fmt.Println(ret)
}

// 查询角色权限
func Test10(t *testing.T) {
    ret, err := enforcer.GetPermissionsForUser(common.SuperAdmin)
    if err != nil {
        panic(err)
    }
    fmt.Println(ret)
...
func Test17(t *testing.T) {
    permList, err := enforcer.GetFilteredPolicy(3, "4")
    if err != nil {
        panic(err)
    }
